...
class Core {
    ...
    async GetPlayerById(id) {
        ...
        const all = await this.Redis.GetHashAll(this._game_session.accessCode),
            player = _.findWhere(all, {
                socket_id: id
            });

        if (!player) {
            logger.log('Could not find player with socket id %s', id);
            return null;
        }
    ...
    PlayerReady(player, socket) {
        ...
        if (!player.connected) {
            ...
            console.log('socket =======>', player.socket_id)
